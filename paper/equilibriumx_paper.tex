...
\subsection{Hybrid Interaction Algorithm}

The core interaction logic is formalized in Algorithm \ref{alg:hybrid}.

\begin{algorithm}
\caption{Hybrid RL-LLM Negotiation Step}
\label{alg:hybrid}
\begin{algorithmic}[1]
\REQUIRE State $s_t$, Policies $\pi_S, \pi_R$, LLM $\mathcal{L}$
\ENSURE Message $m_t$, Next State $s_{t+1}$
\STATE \textbf{RL Step:}
\STATE $a_t^{raw}, p_t \leftarrow \pi(s_t)$ \COMMENT{Sample strategic action}
\STATE \textbf{Translation Step:}
\STATE $c_t \leftarrow \text{ConstructContext}(s_t, a_t^{raw}, p_t)$
\STATE $m_t \leftarrow \mathcal{L}.\text{generate}(c_t | \text{style})$
\STATE \textbf{Environment Step:}
\STATE $s_{t+1}, r_t, d_t \leftarrow \text{Env}.\text{step}(a_t^{raw}, p_t)$
\RETURN $m_t, s_{t+1}$
\end{algorithmic}
\end{algorithm}
...
\begin{itemize}
    ...
    \item Output layer (Multi-Head Action Distribution):
    \begin{itemize}
        \item \textbf{Discrete Head}: Softmax over $A_{type} \in \{\text{ACCEPT}, \text{COUNTER}, \text{QUIT}\}$
        \begin{equation}
        P(a_{type}|s) = \text{softmax}(W_d h_2 + b_d)
        \end{equation}
        \item \textbf{Continuous Head}: Gaussian $\mathcal{N}(\mu, \sigma)$ for Price $p$
        \begin{equation}
        \mu = \text{sigmoid}(W_\mu h_2 + b_\mu) \cdot P_{\max}
        \end{equation}
    \end{itemize}
\end{itemize}
...
A distinguishing feature of EquilibriumX is the Human-in-the-Loop (HITL) override mechanism. While autonomous agents drive the bulk of the negotiation, human experts can interrupt the simulation at any point to manually dictate offers or accept/reject deals. This capability is facilitated through a dedicated WebSocket control channel that allows the backend to pause the RL inference loop and wait for asynchronous human input from the responsive dashboard interface. This feature is particularly valuable for "shadow negotiation" training and adversarial testing of RL-based strategic models.
...
\subsection{Hardening and Deployment Resilience}

To ensure research-grade stability, the system was subjected to chaos testing, including simulated WebSocket disconnections and LLM service latency spikes. The implementation of global exception handlers and multi-stage Docker orchestration ensures that the platform remains resilient under load. The containerized deployment architecture allows for rapid scaling of RL rollout workers in distributed environments while maintaining strict local inference for the LLM layer.
