...
{\footnotesize \textsuperscript{*}IEEE Format Research Paper}
}
...
For a feasible bargaining zone to exist, we require $V_S < V_R$. The difference $(V_R - V_S)$ represents the surplus available for distribution.
...
A price agreement $p^*$ constitutes a Nash Equilibrium if no agent can unilaterally deviate to improve their utility. Formally:

\begin{definition}[Nash Equilibrium Price]
A price $p^* \in [V_S, V_R]$ is a Nash Equilibrium if:
\begin{equation}
U_S(p^*) \geq U_S(p') \quad \forall p' \in [V_S, V_R]
\end{equation}
\begin{equation}
U_R(p^*) \geq U_R(p') \quad \forall p' \in [V_S, V_R]
\end{equation}
\end{definition}

For symmetric bargaining power, the Nash bargaining solution predicts:
\begin{equation}
p^* = \frac{V_S + V_R}{2}
\end{equation}
...
where $\delta \in (0, 1)$ is the discount factor, $\beta$ is a speed bonus, and $\kappa > 0$ is the opportunity cost of no agreement.
...
We formulate the negotiation as a Markov Decision Process (MDP) defined by the tuple $(\mathcal{S}, \mathcal{A}, \mathcal{P}, \mathcal{R}, \gamma)$ where:

\begin{itemize}
    \item $\mathcal{S}$: State space (as defined in Eq. 2)
    \item $\mathcal{A}$: Action space (as defined in Eq. 1)
    \item $\mathcal{P}: \mathcal{S} \times \mathcal{A} \times \mathcal{S} \rightarrow [0, 1]$: Transition probability
...
The LLM Service translates strategic actions into natural language using locally-deployed large language models via Ollama \cite{ollama2024}. This design choice ensures data privacy and eliminates API costs while maintaining low latency.
